...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	/**
	 * @param dependencyType the dependency type to register. This will typically be a base interface such as BeanFactory, with extensions of it resolved
	 *                       as well if declared as an autowiring dependency (e.g. ListableBeanFactory), as long as the given value actually implements the extended interface.
	 ...
	 */
	protected boolean isAutowireCandidate(String beanName, DependencyDescriptor descriptor, AutowireCandidateResolver resolver) throws
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName, @Nullable Set<String> autowiredBeanNames,
		...
		try {// TODO @XX shortcut 是什么？？
			...
			// resolveMultipleBeans method 是解析当前依赖项是否支持多个Bean注入 比如list<>  map<>
			// 如果是能"支持" 多个注入,则在该方法内部就完成了bean的查找,否则下面完成查找 主要是判定你是List<Bean> 还是 bean
			Object multipleBeans = resolveMultipleBeans(descriptor, beanName, autowiredBeanNames, typeConverter);
			if (multipleBeans != null) {
				return multipleBeans;
			}

			// 根据描述符 bean名称,类型 查找候选的匹配Bean
			Map<String, Object> matchingBeans = findAutowireCandidates(beanName, type, descriptor);

			// 如果没找到
			if (matchingBeans.isEmpty()) {
				// 如果是必须注入的 (required default true)
				if (isRequired(descriptor)) {
					// 提示Bean 没有匹配 没有找到 [NoSuchBeanDefinitionException 异常]
			...
			if (matchingBeans.size() > 1) {
				// 通过依赖描述符和候选的Bean 来推断当前被注入的Bean的名称叫什么
				autowiredBeanName = determineAutowireCandidate(matchingBeans, descriptor);
				// 什么是时候为null 当你提供的名称和任何找出来的对象的名字都匹配不上的时候
	...
	private Object resolveMultipleBeans(DependencyDescriptor descriptor, @Nullable String beanName, @Nullable Set<String> autowiredBeanNames,
		...
		if (descriptor instanceof StreamDependencyDescriptor) {
			...
			return result;
		} else if (Collection.class.isAssignableFrom(type) && type.isInterface()) {  // 如果是一个collection 的子类 通常是集合
			...
			return result;
		} else if (Map.class == type) { // 如果是一个Map
	...
	protected Map<String, Object> findAutowireCandidates(@Nullable String beanName, Class<?> requiredType, DependencyDescriptor descriptor) {
		// 通过类型查找符合条件的Bean的名称 Ancestors(祖先)
		...
		if (result.isEmpty()) {
			...
			for (String candidate : candidateNames) {
				if (!isSelfReference(beanName, candidate) && isAutowireCandidate(candidate, fallbackDescriptor) && (!multiple
																													|| getAutowireCandidateResolver().hasQualifier(
						descriptor))) {
					addCandidateEntry(result, candidate, descriptor, requiredType);
				}
			}
	...
	protected String determineAutowireCandidate(Map<String, Object> candidates, DependencyDescriptor descriptor) {
		...
		// 推断优先权 也就是@Order 修饰的Bean 选择最高的返回
		...
		for (Map.Entry<String, Object> entry : candidates.entrySet()) {
			...
			// bean 实例不为空 且解析出来的依赖缓存中包含当前实例, 或者 key()与依赖描述器的依赖名称能匹配上
	...
	private boolean isSelfReference(@Nullable String beanName, @Nullable String candidateName) {
		return (beanName != null && candidateName != null && (beanName.equals(candidateName) || (containsBeanDefinition(candidateName) && beanName.equals(
