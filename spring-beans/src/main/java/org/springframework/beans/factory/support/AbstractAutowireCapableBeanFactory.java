...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory implements AutowireCapableBeanFactory {
	...
	public Object autowire(Class<?> beanClass, int autowireMode, boolean dependencyCheck) throws BeansException {
		...
		if (bd.getResolvedAutowireMode() == AUTOWIRE_CONSTRUCTOR) {
			...
			if (System.getSecurityManager() != null) {
				bean = AccessController.doPrivileged((PrivilegedAction<Object>) () -> getInstantiationStrategy().instantiate(bd, null, this),
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) throws BeanCreationException {
		...
		try {
			...
			// 因为自定义的 target source 不是spring的bean 那么肯定不需要进行后续的一系列的实例化， 初始化
			// 所以可以在 resolveBeforeInstantiation 直接进行proxy
	...
	protected void applyMergedBeanDefinitionPostProcessors(RootBeanDefinition mbd, Class<?> beanType, String beanName) {
		for (BeanPostProcessor bp : getBeanPostProcessors()) {
			if (bp instanceof MergedBeanDefinitionPostProcessor) {
				...
				// 需要依赖注入的成员变量或者方法实例给记录下来 (cache InjectionMetadata), 方便后续populateBean使用
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		if (hasInstAwareBpps) {
			...
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
					// 在这里会对@Autowired @value @inject @resource 等标记的属性或者方法进行依赖注入
	...
	/**
	 *
	 * note : 必须深度复制对象 List<PropertyValue>
	 *
