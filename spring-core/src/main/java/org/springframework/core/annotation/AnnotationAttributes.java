...
/**
 * linkedHashMap的子类 key是方法名称，value 是返回值，在类中实现了一些方法，
 * 方便获取一些基础的类型，简化强转的一个逻辑 例如{@link #getNumber(String)} 等
 * <p>
...
public class AnnotationAttributes extends LinkedHashMap<String, Object> {
	...
	private void assertNotException(String attributeName, Object attributeValue) {
		if (attributeValue instanceof Throwable) {
			throw new IllegalArgumentException(
					String.format("Attribute '%s' for annotation [%s] was not resolvable due to exception [%s]", attributeName, this.displayName,
								  attributeValue), (Throwable) attributeValue);
		}
	}
	...
	private void assertAttributeType(String attributeName, Object attributeValue, Class<?> expectedType) {
		if (!expectedType.isInstance(attributeValue)) {
			throw new IllegalArgumentException(
					String.format("Attribute '%s' is of type %s, but %s was expected in attributes for annotation [%s]", attributeName,
