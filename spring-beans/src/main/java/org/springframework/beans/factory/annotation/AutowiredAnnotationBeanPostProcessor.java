...
public class AutowiredAnnotationBeanPostProcessor extends InstantiationAwareBeanPostProcessorAdapter
	...
	/**
	 * 对于Bean 在扫描阶段进行构建合并BeanDefinition 的同时就缓存了 注入的注解器 InjectMetadata
	 *
		...
		if (candidateConstructors == null) {
			...
			synchronized (this.candidateConstructorsCache) {
				...
				if (candidateConstructors == null) {
					...
					} else if (nonSyntheticConstructors == 2 && primaryConstructor != null && defaultConstructor != null && !primaryConstructor.equals(
							defaultConstructor)) {
	...
	public PropertyValues postProcessProperties(PropertyValues pvs, Object bean, String beanName) {
		// 根据当前Bean 查找出所有加了@Autowired 的属性
		// 然后根据这些属性实例化一个注入器
	...
	/**
	 * 不太懂，injectionMetadataCache　本身就是 ConcurrentHashMap 为何还要加 Synchronize(this.injectionMetadataCache)
	 */
	...
	private InjectionMetadata buildAutowiringMetadata(Class<?> clazz) {
		...
		do {
			// 存放所有需要注入的元素   字段(AutowiredFieldElement) 方法(AutowiredMethodElement)
			final List<InjectionMetadata.InjectedElement> currElements = new ArrayList<>();

			// 查询所有标记了@Autowired 注解的字段
			ReflectionUtils.doWithLocalFields(targetClass, field -> {
				// 如果这里 ann 为null 但是我们原本的类中没有，是因为扎里的对象是经过了cglib 代理的，已经不是原本自己写的那个样子了
				MergedAnnotation<?> ann = findAutowiredAnnotation(field);
				if (ann != null) {
					// 这里体现了spring 不支持static field 注入
			...
			// 查询所有标记了@Autowired 注解的方法， 依次向上查询，递归向上，找到所有打了注解的字段
			ReflectionUtils.doWithLocalMethods(targetClass, method -> {
	...
	private class AutowiredFieldElement extends InjectionMetadata.InjectedElement {
		...
		/**
		 * 整体注入的思路，是先查询cache  cache 有则直接反射注入
		 * cache 没有，则进行resolveFieldValue 解析
		 * 找到 反射注入
		 */
		@Override
